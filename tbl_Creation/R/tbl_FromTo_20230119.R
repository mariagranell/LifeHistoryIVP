...
# One issue that we found we Miguel is that CurrentGroup must be updated. The consequence of animals having
# CurrentGroup != NA whereas they have disappeared from the population but do not have a closing
# date (NatalEmigrationDate,LastDate1,2,3,4) lead to the assigment of these animals to the alive category.
# Miguel will send an email to the project and ask about it


# To do -------------------------------------------------------------------

# GroupCount variable: check the following; a non-natal male immigrant arriving in a group as immigrant 1 data
# was assigned group of 1.


# Paths ------------------------------------------------------------------

# input file is either animalID, animalCode
...
tbl_AnimalID <- read.csv(input_table)
head(tbl_AnimalID)

# tbl_LifeHistory ---------------------------------------------------------
input_table <- "tbl_LifeHistory.csv"
tbl_LifeHistory <- read.csv(input_table)
str(tbl_LifeHistory)

# tbl_Sex -----------------------------------------------------------------
input_table <- "tbl_Sex.csv"
tbl_Sex <- read.csv(input_table)


# tbl_DOB -----------------------------------------------------------------
input_table <- "tbl_DOB.csv"
tbl_DOB <- read.csv(input_table)

# tbl_Natal -----------------------------------------------------------------
input_table <- "tbl_Natal.csv"
tbl_DOB <- read.csv(input_table)
...
Natal_DOB_NoEmi_WithEmiDate <- Natal_DOB_NoEmi %>% 
       filter(!is.na(EmigrationNatalDate))
View(Natal_DOB_NoEmi_WithEmiDate)
...
View(Natal_DOB_NoEmi_WithEmiDate_SameDate)
...
View(Natal_DOB_NoEmi_WithEmiDate_DiffDate)
#5 entries where emigration natal date != Last date 1
#The dates seems pretty close
...
View(FromTo_MissingID_Info %>% 
       filter(!is.na(FirstDate)))
#20 of these animals seems to be missing a first date 
#Thus we would not be able to assign first date to these 29 animals


#DOB
View(FromTo_MissingID_Info %>% 
       filter(!is.na(DOB)))
#2 of these animals seems to be born in the population
# They should be natal == yes


# DC missing natal --------------------------------------------------------
#Animals present in tbl_Natal and absent from tbl_FromTo
#That should be corrected pretty soon probably as some of them may be of interest for natal dispersion paper

#tbl_FromTo: I manually entered NatalGroup = "Yes" for animals for which the rationale of the tbl suggested they were in their natal group
names(tbl_FromTo)

#tbl_Natal: Natal animals are queried from LHF with non null DOB or BirthGroup. It simply suggests that an animal was born somwhere know 
names(tbl_Natal)


#Animals present in natal but do not appear in the tbl_FromTo
Natal_NoNatalGroup <- anti_join(tbl_Natal %>% filter(Natal == "Yes"),
                                tbl_FromTo %>% filter(NatalGroup == "Yes"), 
  ...
  left_join(.,tbl_LifeHistory %>% 
              select(AnimalID_Std,
                     ...
                     ImmigrationGroup1,
                     ImmigrationDate1,
                     LastDate1), by=c("AnimalID" = "AnimalID_Std"))
View(Natal_NoNatalGroup)
#24 animals 
#Only 2 of them have a DOB
#Thus they should all fall somewhere in the 



# DC Missing group --------------------------------------------------------
# Take from compute relevant variables in DataPrep_Immigration_20230119b
...
# DC Mismatch group nb ----------------------------------------------------
# mismatch between group nb from tbl_FromTo and computed from row
#Shoud have mismatch for all animals that have skipped a group in the sequence or are lacking group 1
